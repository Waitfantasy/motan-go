...
func BuildHeader(msgType int, proxy bool, serialize int, requestID uint64, msgStatus int) *Header {
	...
	// 消息状态0 正常消息，1 请求处理异常。共支持2^3即8种状态。request和response可以分别表示不同状态，其他状态待扩展
	//status
	status := uint8(0x08 | (uint8(msgStatus) & 0x07))

	serial := uint8(0x00 | (uint8(serialize) << 3))

	// serialize: 0 hessian，1 grpc-pb，2 json，3 msgpack，4 hprose，5 pb，6 simple，7 grpc-pb-json，其他待扩展

	// requestID: long型requestId。requestID需要保证对同一节点请求唯一。重复请求时可以使用相同requestId
